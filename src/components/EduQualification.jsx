...
const Qualification = () => {
    ...
    return (
        <section
            id="qualification"
            className="relative w-full min-h-fit md:min-h-[92vh] bg-[#030014] text-white overflow-hidden flex items-center py-8 md:py-0"
        >
            <div className="hidden md:block">
                {desktopGeometricShapes.map((shape, idx) => (
                    <motion.div
                        ...
                        animate={{ y: [0, -30, 0, 30, 0], x: [0, 20, 0, -20, 0], rotate: [0, 360] }}
                        transition={{ duration: shape.duration, delay: shape.delay, repeat: Infinity, ease: "easeInOut" }}
                    >
                        {shape.type === "triangle" && (
                            <div className="border-l-transparent border-r-transparent border-b-purple-500/30"
                                style={{ width: 0, height: 0, borderLeftWidth: shape.size / 2, borderRightWidth: shape.size / 2, borderBottomWidth: shape.size }} />
                        )}
                        {shape.type === "square" && <div className="w-full h-full border-2 border-purple-500/30 rotate-12" />}
                        {shape.type === "circle" && <div className="w-full h-full rounded-full border-2 border-purple-500/30" />}
                        {shape.type === "diamond" && <div className="w-full h-full border-2 border-purple-500/30 rotate-45" />}
                        {shape.type === "hexagon" && <div className="w-full h-full bg-purple-500/20" style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }} />}
                    </motion.div>
                ))}
            </div>
            ...
            <div className="md:hidden">
                {mobileGeometricShapes.map((shape, idx) => (
                    <motion.div
                        ...
                        style={{ top: shape.top, left: shape.left, width: shape.size, height: shape.size }}
                        animate={{ y: [0, -15, 0, 15, 0], x: [0, 10, 0, -10, 0], rotate: [0, 180] }}
                        transition={{ duration: shape.duration, delay: shape.delay, repeat: Infinity, ease: "easeInOut" }}
                    >
                        {shape.type === "triangle" && (
                            <div className="border-l-transparent border-r-transparent border-b-purple-500/30"
                        ...
                        {shape.type === "circle" && <div className="w-full h-full rounded-full border border-purple-500/30" />}
                        {shape.type === "square" && <div className="w-full h-full border border-purple-500/30 rotate-12" />}
                    </motion.div>
                ))}
            </div>
            ...
            <div className="relative max-w-5xl mx-auto w-full px-5 md:px-8 py-6 md:py-10">
                <motion.div
                ...
                >
                    <h2 className="text-gray-200 text-3xl md:text-5xl font-extrabold">Qualification</h2>
                    <p className="text-gray-400 text-xs md:text-sm mt-2">My Academic Journey</p>
                </motion.div>
                                ...
                                >
                                    <div className="md:hidden relative pl-[50px]">
                                        ...
                                        <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-4 backdrop-blur-xl hover:border-purple-500/30 transition-all duration-300">
                                            <h3 className="text-sm font-bold leading-snug text-white">
                                                {item.degree}, <span className="text-purple-400">{item.year}</span>
                                            </h3>
                                            <p className="text-gray-300 text-xs mt-1.5 font-medium">{item.institution}</p>
                                            <div className="flex items-center justify-between mt-3">
                                                <div className="flex items-center gap-1 text-gray-400 text-[11px]">
                                                    <CalendarDays size={12} /> {item.group}
                                                </div>
                                                <div className="flex items-center gap-1 text-purple-400 text-[11px] font-semibold">
                                                    <Award size={12} /> {item.result}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`hidden md:block w-[44%] ${isLeft ? 'mr-auto' : 'ml-auto'} relative`}>
                                        <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-4 backdrop-blur-xl hover:border-purple-500/30 transition-all duration-300">
                                            ...
                                            <p className="text-gray-300 text-xs md:text-sm mt-2 font-medium">{item.institution}</p>
                                            <div className="flex items-center justify-between mt-3">
